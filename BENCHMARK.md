# ChatJimmy Speed Benchmark — "The Fastest Coding Harness"

Claude Code driving ChatJimmy's Llama 3.1 8B backend through the proxy.
Reproduce: `./eval/speed-benchmark.sh` (proxy must be up on :3000 in ChatJimmy mode).

## Raw inference speed (ChatJimmy `/api/chat` telemetry)
...
- Claude / GPT-4 class: ~50–100 tok/s → **ChatJimmy is ~150–280× faster**
- Groq Llama-3.1-8B (famously fast): ~1,250 tok/s → **ChatJimmy is ~10× faster**
...
A frontier model spends **~2–5 s per turn** on inference alone; ChatJimmy spends ~332 ms
...
On a normal model, 5× sampling would be prohibitively slow. On ChatJimmy, each draw is
...
ChatJimmy's instance is more quantized than OpenRouter's, so absolute quality is lower,
...
```bash
cd your-project
/Users/dennison/develop/chatjimmy-proxy/chatjimmy-code -p "your coding task"
# or interactively:
/Users/dennison/develop/chatjimmy-proxy/chatjimmy-code
```
